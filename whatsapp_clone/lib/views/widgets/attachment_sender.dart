...
class _AttachmentMessageSenderState extends State<AttachmentMessageSender> {
  ...
  Future<void> addNewAttachments() async {
    ...
    if (current.type == AttachmentType.document) {
      newAttachments = await widget.pickDocuments(
        context,
      ...
      newAttachments = await widget.pickAttachmentsFromGallery(
        context,
        returnAttachments: true,
      );
    }
  ...
  Future<void> sendAttachments() async {
    for (var i = 0; i < controllers.length; i++) {
      ...
      if (msgContent.isEmpty && attachment.type == AttachmentType.document.name) {
        msgContent = "\u00A0";
      }
      ...
      sendMessageWithAttachments(
        Message(
          ...
          screenName: widget.self.screenName,
          sendingDate: DateTime.now().microsecondsSinceEpoch * 1000,
          mediaUrls: {},
          preview: attachment.type == AttachmentType.image.name, systemMessage: false,
        ),
      );
    }
