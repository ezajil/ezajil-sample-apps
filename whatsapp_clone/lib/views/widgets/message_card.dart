...
class _MessageCardState extends State<MessageCard>
  ...
  Widget build(BuildContext context) {
    ...
    final hasAttachment = widget.message.mediaUrls != null;
    final attachmentType = widget.message.preview ? 'IMAGE' : 'DOCUMENT';
    final isSentMessageCard = widget.currentUserId == widget.message.author;
    final messageHasText = widget.message.content.isNotEmpty;

    final showTimeStamp = !hasAttachment ||
        (hasAttachment && attachmentType == 'AUDIO' && messageHasText) ||
        ((hasAttachment && attachmentType != 'AUDIO') &&
            (hasAttachment && attachmentType != 'VOICE'));
    ...
    final hasImageOrVideo = widget.message.preview;
    final maxWidth = size.width * 0.80;
    final maxHeight = size.height * 0.40;
    final minWidth = 0.8 * maxWidth;
    const imgWidth = 20.0;
    const imgHeight = 20.0;
    ...
    double minWidth2 = widget.special ? (isSentMessageCard ? 98 : 76) : 60;
    double maxWidth2 =
        hasImageOrVideo ? width : size.width * 0.80 + (widget.special ? 10 : 0);

    // Ensure maxWidth is always at least as large as minWidth
    maxWidth2 = max(minWidth, maxWidth);
      ...
      child: ClipPath(
        ...
        child: Container(
          ...
          padding: hasAttachment
              ? attachmentType == 'AUDIO' && !messageHasText
                  ? EdgeInsets.only(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      if (isSentMessageCard) ...[
                        ...
                        Image.asset(
                          'assets/images/${widget.message.status.name}.png',
                          color: widget.message.status.name != 'READ'
                              ? messageHasText
                                  ? colorTheme.textColor1
                                      .withOpacity(0.65)
