...
class _ChatStreamState extends ConsumerState<ChatStream> {
  ...
  void initState() {
    ...
    chatId = getChatId(self.userId, other.userId);
  ...
  void handleNewMessage(Message message) {
    if (message.author == self.userId) {
      unreadCount = 0;
      if (message.status == MessageStatus.notSent) {
        scrollToBottom();
      }
    } else {
  ...
  int updateUnreadCount(List<Message> messages) {
    ...
    for (final message in messages) {
      if (message.author == self.userId) break;
  ...
  void markAsSeen(Message message) {
    if (message.author == self.userId) return;
                  ...
                  child: Center(
                    child: ChatDate(
                      date: messages.isEmpty
                          ? 'Today'
                          : dateFromTimestamp(messages.last.sendingDate),
                    ),
                  ),
                ),
              ],
  ...
  Widget buildMessageCard(int index, List<Message> messages) {
    ...
    final currMsgDate = dateFromTimestamp(messages[index].sendingDate);
    final showDate = isFirstMsg ||
        currMsgDate != dateFromTimestamp(messages[index + 1].sendingDate);
