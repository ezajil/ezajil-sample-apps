...
class _AttachedImageVideoViewerState
  ...
  void initState() {
    ...
    final clientIsSender = widget.message.author == self.userId;
    sender = clientIsSender ? "You" : other.screenName;
...
class _AttachedVoiceViewerState extends ConsumerState<AttachedVoiceViewer> {
  ...
  void initState() {
    ...
    clientIsSender = widget.message.author == self.userId;
    ...
    playerStateListener = player.onPlayerStateChanged.listen((event) async {
      if (Platform.isAndroid && event == PlayerState.completed) {
        ...
        remainingDuration = ((1 - progressNotifier.value) *
    ...
    if (clientIsSender) {
      avatarUrl = self.avatarUrl ?? 'https://en.gravatar.com/userimage/238463648/8cc16f6f5423605920569a634fd097eb.jpeg?size=256';
    } else {
      final other = ref.read(chatControllerProvider.notifier).other;
      avatarUrl = other.avatarUrl ?? 'https://en.gravatar.com/userimage/238463648/8cc16f6f5423605920569a634fd097eb.jpeg?size=256';
    }
  ...
  Widget build(BuildContext context) {
    ...
    final micColor = widget.message.status == MessageStatus.read
...
class _AttachmentViewerState extends State<AttachmentViewer> {
  ...
  Widget build(BuildContext context) {
    ...
    String formattedTime = formattedTimestamp(widget.message.sendingDate);
...
class _DownloadingAttachmentState extends ConsumerState<DownloadingAttachment> {
  ...
  void initState() {
    ...
    clientIsSender = ref.read(chatControllerProvider.notifier).self.userId ==
...
class _UploadingAttachmentState extends ConsumerState<UploadingAttachment> {
  ...
  void initState() {
    ...
    if (isUploading) {
      final stream = UploadService.getUploadStream(widget.message.messageId);
      if (stream != null) {
        uploadStream = stream;
      }
    }
  ...
  void didUpdateWidget(covariant UploadingAttachment oldWidget) {
    ...
    if (isUploading) {
      uploadStream = UploadService.getUploadStream(widget.message.messageId)!;
    }
  }
